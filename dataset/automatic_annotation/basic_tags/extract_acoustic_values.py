...
#
# Extracts acoustic metadata values (pitch, speaking rate, SNR, reverberation)
# from a HuggingFace dataset and saves the enriched dataset to disk.
#
# Usage:
#   python extract_acoustic_values.py ./my-dataset \
#       --load_from_disk \
#       --output_dir ./my-dataset-with-acoustic-values \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--audio_path_column_name", default="audio_path", type=str, help="Column name of the column that contains the path to the audio, fallback if the audio column is absent")
    ...
    audio_path_column_name = "audio_path" if args.rename_column else args.audio_path_column_name
    text_column_name = "text" if args.rename_column else args.text_column_name

    if isinstance(dataset.column_names, dict):
        column_names = dataset.column_names[list(dataset.column_names.keys())[0]]
    else:
        column_names = dataset.column_names

    assert args.audio_column_name in column_names or args.audio_path_column_name in column_names, f"Audio column name {args.audio_column_name} or {args.audio_path_column_name} not found in the dataset."
    assert args.text_column_name in column_names, f"Text column name {args.text_column_name} not found in the dataset."

    if args.rename_column:
        if args.audio_column_name in column_names:
            dataset = dataset.rename_columns({args.audio_column_name: "audio"})
        if args.audio_path_column_name in column_names:
            dataset = dataset.rename_columns({args.audio_path_column_name: "audio_path"})
        dataset = dataset.rename_columns({args.text_column_name: "text"})

    if audio_column_name not in column_names:
        dataset = dataset.map(lambda x: {audio_column_name: x[audio_path_column_name]}).cast_column(audio_column_name, Audio())
