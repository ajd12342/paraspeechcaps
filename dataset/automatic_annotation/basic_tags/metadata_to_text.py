...
#
# Converts continuous acoustic metadata values (pitch, speaking rate, SNR) to
# categorical text labels using pre-computed bin edges.
#
# Usage (with ParaSpeechCaps bin configuration):
#   python metadata_to_text.py ./my-dataset-with-acoustic-values-and-gender \
#       --load_from_disk \
#       --output_dir ./my-dataset-with-basic-tags \
#       --cpu_num_workers 8 \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_from_disk", default=False, action="store_true", help="If `True`, will load the dataset from disk.")
    ...
    parser.add_argument("--save_bin_edges", default=None, type=str, help="If specified, it's the name of the JSON file which will contains the edge bins that have been computed. Useful if you want to reuse those bin edges on new datasets. By default, it won't save those edges.")
    parser.add_argument("--avoid_pitch_computation", default=False, action="store_true", help="If `True`, will not compute `pitch`. Note that `pitch` is computed on a speaker-level, so you don't need it in a mono-speaker setting.")
    ...
    parser.add_argument("--pitch_std_tolerance", default=2., type=float, help="Standard deviation tolerance for pitch estimation. Any value that is outside mean ± std * tolerance is discarded. Only used if `avoid_pitch_computation=False`.")
    parser.add_argument("--speaking_rate_std_tolerance", default=4., type=float, help="Standard deviation tolerance for speaking rate estimation. Any value that is outside mean ± std * tolerance is discarded. Only used if `path_to_bin_edges=False`.")
    parser.add_argument("--snr_std_tolerance", default=3.5, type=float, help="Standard deviation tolerance for SNR estimation. Any value that is outside mean ± std * tolerance is discarded. Only used if `path_to_bin_edges=False`.")
    parser.add_argument("--reverberation_std_tolerance", default=4, type=float, help="Standard deviation tolerance for reverberation estimation. Any value that is outside mean ± std * tolerance is discarded. Only used if `path_to_bin_edges=False`.")
    parser.add_argument("--speech_monotony_std_tolerance", default=4, type=float, help="Standard deviation tolerance for speech monotony estimation. Any value that is outside mean ± std * tolerance is discarded. Only used if `path_to_bin_edges=False`.")
    ...
    if args.only_save_plot and args.path_to_bin_edges:
        raise ValueError("`only_save_plot=true` but `path_to_bin_edges` is specified. Since the latter is specified, we won't redo computations that would have been used for plotting. Chose one or another. Note that if you use this script to label a new dataset for fine-tuning, I'd recommend avoiding plotting and set `only_save_plot=false`")
